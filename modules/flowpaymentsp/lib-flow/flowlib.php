...
function flow_servipag_get_endpoint() {
	return 'https://www.flow.cl/app/kpf/pago.php';
}

function flow_servipag_get_endpoint_test() {
	return 'http://flow.tuxpan.com/app/kpf/pago.php';
}

function flow_servipag_get_parameter($key) {
	global $_GET;
	return isset($_GET[$key])?$_GET[$key]:null;
}

function flow_servipag_get_post_parameter($key) {
	global $_POST;
	return isset($_POST[$key])?$_POST[$key]:null;
}

function flow_servipag_get_private_key_id($priv_key_data) {
	return openssl_get_privatekey($priv_key_data);
}

function flow_servipag_get_public_key_id() {
...
function flow_servipag_sign($priv_key, $data) {
	$priv_key_id = flow_servipag_get_private_key_id($priv_key);
	openssl_sign($data, $signature, $priv_key_id);
	return base64_encode($signature);
}

function flow_servipag_sign_validate($data) {
	$params = array();
	...
	$pub_key_id = flow_servipag_get_public_key_id();
...
function flow_servipag_pack($privkey, $flow_comercio, $orden_compra, $monto, $medioPago, $concepto, $id_producto, 
	...
	$signature = flow_servipag_sign($privkey, $p);
	return $p."&s=$signature";
}

function flow_servipag_build_response($priv_key, $data) {
	$q = http_build_query($data);
	$s = flow_servipag_sign($priv_key, $q);
	return $q."&s=".$s;
}

function flow_servipag_post($url, $data) {
	$ch = curl_init($url);
	...
	$data = flow_servipag_sign_validate($response);
	if (isset($data['error'])) throw new Exception($data['error']);
	return $data;
}

function flow_servipag_request($service, $params) {
	global $flow_url_pago;
	$data = flow_servipag_pack($params);

	return flow_servipag_post($flow_url_pago, $data);
}

function flow_servipag_sign_validate_post() {
	$data = file_get_contents('php://input');
	return flow_servipag_sign_validate($data);
}

function flow_servipag_aget($a, $key) {
