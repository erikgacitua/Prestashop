...
<h2>{l s='Pago via Servipag usando Flow' mod='flowpaymentsp'}</h2>

<fieldset>
    <legend><img src="../img/admin/warning.gif"/>{l s='Information' mod='flowpaymentsp'}</legend>
...
<form action="{$post_url}" method="post" enctype="multipart/form-data" style="clear: both; margin-top: 10px;">
    <fieldset>
	    ...
	    <label for="platformType">{l s='Plataforma de Flow' mod='flowpaymentsp'}</label>
        ...
        <label for="title">{l s='Nombre del medio de pago' mod='flowpaymentsp'}</label>
        ...
        <label for="idComercio">{l s='ID Comercio Flow' mod='flowpaymentsp'}</label>
        ...
        {/if}
		<label for="privateKey">{l s='Llave privada Flow' mod='flowpaymentsp'}</label>

        <div class="margin-form">{$data_privateKeyInfo}<input type="file" name="privateKey" /></div>
        
	    <label for="skipType">{l s='Modo de acceso a Webpay' mod='flowpaymentsp'}</label>
	    <div class="margin-form">
		    <select name="skipType">
		    	<option value="d" {if $data_skipType eq "d"}selected{/if}>Ingresar directamente a Servipag</option>
        ...
        <label for="additional">{l s='Cobro adicional (en %)' mod='flowpaymentsp'}</label>

        <div class="margin-form"><input type="text" size="5" id="additional" name="additional"
                                        value="{$data_additional}"/></div>
                                        
        <center><input type="submit" name="flow_updateSettings" value="{l s='Save Settings' mod='flowpaymentsp'}"
